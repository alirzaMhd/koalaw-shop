...
(function () {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const SAMPLE_ITEMS = [
      { id: "p-100", title: "اکسیر درخشش طلایی", price: 480000, qty: 1, image: "./src/assets/images/product.png", variant: "30ml" },
      { id: "p-200", title: "رژ لب بوسه مخملی", price: 320000, qty: 2, image: "./src/assets/images/cosmetic.png", variant: "Shade 03" },
    ];
    const RECS = [
      { id: "r-1", title: "سرم هیالورونیک", price: 290000, image: "./src/assets/images/skin.png" },
      { id: "r-2", title: "عطر راز نیمه‌شب", price: 780000, image: "./src/assets/images/perfume.png" },
      { id: "r-3", title: "ماسک مو تغذیه‌کننده", price: 350000, image: "./src/assets/images/hair.png" },
      { id: "r-4", title: "رژ گونه لطیف", price: 260000, image: "./src/assets/images/cosmetic.png" },
    ];
    ...
    function saveCart(cart) { KUtils.setJSON(CART_KEY, cart); }
    ...
    function saveState(state) { KUtils.setJSON(STATE_KEY, state); }

    function loadAddress() {
      return KUtils.getJSON(ADDR_KEY, { firstname:"", lastname:"", phone:"", postal:"", province:"", city:"", line1:"" });
    }
    function saveAddress(addr) { KUtils.setJSON(ADDR_KEY, addr); }
    ...
    function updateHero(route) {
      const meta = heroMap[route]; if (!meta) return;
      ...
      if (icon) { icon.setAttribute("data-feather", meta.icon); icon.replaceWith(icon.cloneNode(false)); KUtils.refreshIcons(); }
    }
    ...
    function updateStepper(route, cart, addr) {
      ...
      [sc, sa, sp].forEach(s => { if (!s) return; s.classList.remove("is-done","is-current"); s.removeAttribute("aria-disabled"); });
      ...
      if (!hasCart) { sa?.setAttribute("aria-disabled","true"); sp?.setAttribute("aria-disabled","true"); }
    ...
    function renderCart() {
      ...
      couponMsg && (couponMsg.textContent = state.coupon ? (coupons[state.coupon]?.msg || "کد اعمال‌شده.") : "");

      const navCnt = document.getElementById("nav-cart-count");
      if (navCnt) navCnt.textContent = KUtils.toFa(cart.reduce((a, c) => a + c.qty, 0));
      ...
      if (!cart.length) {
        ...
        cart.forEach(line => {
          ...
          row.className = "order-card"; row.dataset.id = line.id;
          row.innerHTML = `
      ...
      // handlers
      itemsWrap?.querySelectorAll(".qty-btn.plus").forEach(btn => btn.addEventListener("click", () => {
        const id = btn.closest("[data-id]").dataset.id;
        const i = cart.findIndex(x => x.id === id);
        if (i > -1) { cart[i].qty++; saveCart(cart); renderCart(); }
      }));
      itemsWrap?.querySelectorAll(".qty-btn.minus").forEach(btn => btn.addEventListener("click", () => {
        const id = btn.closest("[data-id]").dataset.id;
        const i = cart.findIndex(x => x.id === id);
        if (i > -1) { cart[i].qty = Math.max(1, cart[i].qty - 1); saveCart(cart); renderCart(); }
      }));
      itemsWrap?.querySelectorAll(".qty-input").forEach(inp => inp.addEventListener("input", () => {
        const row = inp.closest("[data-id]"); const id = row.dataset.id;
        const i = cart.findIndex(x => x.id === id);
        let v = parseInt(inp.value || "1", 10); if (isNaN(v) || v < 1) v = 1;
        if (i > -1) {
          cart[i].qty = v; saveCart(cart);
          row.querySelector("[data-line-total]").textContent = toIRR(cart[i].price * cart[i].qty);
          updateCartSummary();
        }
      }));
      itemsWrap?.querySelectorAll(".remove").forEach(btn => btn.addEventListener("click", () => {
        const id = btn.closest("[data-id]").dataset.id;
      ...
      function updateCartSummary() {
        const s = loadState(); const c = loadCart(); const t = computeTotals(c, s);
        ...
        discount && (discount.textContent = t.discount > 0 ? `- ${toIRR(t.discount)}` : "۰ تومان");
        shipping && (shipping.textContent = t.shipping === 0 ? "رایگان" : toIRR(t.shipping));
        ...
        if (current >= FREE_SHIP_THRESHOLD) {
          progText && (progText.textContent = "تبریک! ارسال رایگان برای سفارش شما فعال شد.");
          if (shipBadge) {
          ...
          progText && (progText.textContent = `برای ارسال رایگان، هنوز ${toIRR(remain)} مانده است.`);
          if (shipBadge) {
      ...
      // Options
      ex && (ex.onchange = (e) => { const s = loadState(); s.express = e.target.checked; s.shippingMethod = s.express ? "express" : "standard"; saveState(s); updateCartSummary(); });
      gw && (gw.onchange = (e) => { const s = loadState(); s.gift = e.target.checked; saveState(s); updateCartSummary(); });
      note && (note.oninput  = (e) => { const s = loadState(); s.note = e.target.value; saveState(s); });
      ...
      applyBtn && (applyBtn.onclick = () => {
        ...
        const subtotal = cart.reduce((a, c) => a + c.price * c.qty, 0);
        if (!code) { const s = loadState(); s.coupon = ""; saveState(s); couponMsg.textContent = ""; updateCartSummary(); return; }
        ...
        if (r) {
          if (subtotal < r.min) {
            ...
            const s = loadState(); s.coupon = code; saveState(s);
      ...
      clearBtn && (clearBtn.onclick = () => {
        ...
        if (confirm("آیا از پاک کردن سبد خرید مطمئن هستید؟")) { saveCart([]); renderCart(); }
      });
      ...
      recWraps.forEach(recWrap => {
        ...
        RECS.forEach(p => {
          ...
          d.addEventListener("click", () => {
            ...
            if (i > -1) c[i].qty += 1;
            else c.push({ id: p.id, title: p.title, price: p.price, qty: 1, image: p.image, variant: "" });
    ...
    function renderAddress() {
      ...
      // Prefill + inputs
      const fields = ["firstname","lastname","phone","postal","province","city","line1"];
      fields.forEach(k => {
        const el = document.getElementById("addr-" + k);
        if (!el) return;
        el.value = addr[k] || "";
        el.addEventListener("input", () => { addr[k] = el.value.trim(); saveAddress(addr); update(); });
      });
      ...
      if (std && exp) {
        ...
        document.getElementById("addr-price-standard").textContent = KUtils.toIRR(45000);
        document.getElementById("addr-price-express").textContent = KUtils.toIRR(45000 + 30000);
        document.querySelectorAll('input[name="addr-ship"]').forEach(r => {
          r.onchange = () => { state = loadState(); state.express = r.value === "express"; state.shippingMethod = state.express ? "express" : "standard"; saveState(state); update(); };
        });
      }
      ...
      function update() {
        ...
        set("addr-discount", t.discount > 0 ? `- ${KUtils.toIRR(t.discount)}` : "۰ تومان");
        set("addr-shipping", t.shipping === 0 ? "رایگان" : KUtils.toIRR(t.shipping));
        set("addr-grand", KUtils.toIRR(t.total));
        const a = loadAddress();
        set("addr-ship-to", a.firstname ? `${a.firstname} ${a.lastname} — ${a.phone}\n${a.province}، ${a.city}، ${a.line1} — کد پستی: ${a.postal || "—"}` : "—");
        const navCnt = document.getElementById("nav-cart-count");
        if (navCnt) navCnt.textContent = KUtils.toFa(loadCart().reduce((x, y) => x + y.qty, 0));
      }
      update();
      KUtils.refreshIcons();
      ...
      next && (next.onclick = () => {
        addr = loadAddress();
        if (!validateAddress(addr)) { showToast("لطفاً اطلاعات آدرس را کامل کنید", "alert-circle"); return; }
        navigate("payment");
      });
    }
    ...
    function renderPayment() {
      ...
      function update() {
        ...
        set("pay-discount", t.discount > 0 ? `- ${KUtils.toIRR(t.discount)}` : "۰ تومان");
        set("pay-shipping", t.shipping === 0 ? "رایگان" : KUtils.toIRR(t.shipping));
        set("pay-grand", KUtils.toIRR(t.total));
        const a = loadAddress();
        set("pay-ship-to", `${a.firstname} ${a.lastname} — ${a.phone}\n${a.province}، ${a.city}، ${a.line1} — کد پستی: ${a.postal || "—"}`);
        const navCnt = document.getElementById("nav-cart-count");
        if (navCnt) navCnt.textContent = KUtils.toFa(loadCart().reduce((x, y) => x + y.qty, 0));
      }
      update();
      ...
      radios.forEach(r => r.addEventListener("change", () => { state = loadState(); state.payMethod = r.value; KUtils.setJSON(STATE_KEY, state); }));
      ...
      payBtn && (payBtn.onclick = () => {
        if (locking) return;
        const method = (document.querySelector('input[name="pay-method"]:checked')?.value) || "gateway";
        state = loadState(); state.payMethod = method; KUtils.setJSON(STATE_KEY, state);
        if (method === "gateway") {
          locking = true;
          ...
          payBtn.disabled = true; payBtn.classList.add("opacity-80","cursor-not-allowed");

          // Simulate redirect + return
          setTimeout(() => {
            setTimeout(() => {
              payBtn.innerHTML = payBtn.dataset.orig || "پرداخت و ثبت سفارش";
              payBtn.disabled = false; payBtn.classList.remove("opacity-80","cursor-not-allowed");
              locking = false;
