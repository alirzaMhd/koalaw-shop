...
(function () {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Quantity
    let quantity = 1;
    function renderQty() { qtyDisplay && (qtyDisplay.textContent = KUtils.toFa(quantity)); }
    renderQty();
    ...
    // Map buttons (requires small HTML tweak to add data-tab)
    tabButtons.forEach(btn => btn.addEventListener("click", (e) => {
      e.preventDefault();
      
      if (name) showTabByName(name);
    }));
    ...
    function setupRatingControl() {
      ...
      const paint = (value) => stars.forEach((s, i) => { s.style.fill = i < value ? "currentColor" : "none"; });
      stars.forEach((star, idx) => {
        ...
        star.addEventListener("click", () => { currentRating = idx + 1; paint(currentRating); });
        star.addEventListener("keydown", (e) => {
          if (e.key === "Enter" || e.key === " ") { e.preventDefault(); currentRating = idx + 1; paint(currentRating); }
        });
      });
    }
    setupRatingControl();
    ...
    reviewForm && reviewForm.addEventListener("submit", (e) => {
      e.preventDefault();
      if (currentRating < 1) { alert("لطفاً امتیاز بدهید."); return; }
      const name = reviewName.value.trim();
      const text = reviewText.value.trim();
      if (!name || !text) return;

      const starsHtml = Array(5).fill(0).map((_, i) =>
        `<i data-feather="star" class="w-4 h-4 ${i < currentRating ? "fill-current" : ""}"></i>`).join("");

      const avatarSeed = Math.random() > 0.5 ? "1494790108377-be9c29b29330" : "1507003211169-0a1dd7228f2d";
      ...
      // Reset
      reviewForm.reset(); currentRating = 0; setupRatingControl();
      reviewMsg && (reviewMsg.classList.remove("hidden"), setTimeout(() => reviewMsg.classList.add("hidden"), 2000));

      userReviewsCount++; updateReviewCounts();
    });

    // Fill stars in "related products" cards
    document.querySelectorAll(".product-card-v3 .card-rating").forEach((r) => {
      const vEl = r.querySelector("span"); if (!vEl) return;
      const ratingValue = parseFloat((vEl.textContent || "").replace("٫","."));
      const stars = r.querySelectorAll('i[data-feather="star"]');
      stars.forEach((s, i) => { if (i < Math.floor(ratingValue)) s.classList.add("fill-current"); });
    });
    KUtils.refreshIcons();
  });
})();
