...
async function main() {
    ...
    const colorThemes = await Promise.all(colorThemeInputs.map((d) => prisma.colorTheme.upsert({
        where: { slug: d.slug },
    ...
    await Promise.all(users.map((u) => prisma.userNotificationPrefs.upsert({
        where: { userId: u.id },
        update: {},
        create: {
            userId: u.id,
    ...
    const brands = await Promise.all(brandInputs.map((b) => prisma.brand.upsert({
        where: { slug: b.slug },
        update: { name: b.name },
        create: b,
    })));
    ...
    const dbCategories = await Promise.all(dbCategoryInputs.map((c) => prisma.category.upsert({
        where: { value: c.value },
    ...
    const magCategories = await Promise.all(magCatInputs.map((c) => prisma.magazineCategory.upsert({
        where: { code: c.code },
