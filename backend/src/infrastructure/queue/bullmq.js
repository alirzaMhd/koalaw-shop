...
export function bindDefaultEmailWorker() {
  ...
  try {
    ...
    const mod = require("../mail/mailer.js");
    ...
    emailWorker = new Worker(
      ...
      {
        connection: connection(),
        concurrency: Number(env.QUEUE_EMAIL_CONCURRENCY || 5),
      }
    );
    emailWorker.on("failed", (job, err) =>
      logger.warn({ jobId: job?.id, err }, "email worker failed")
    );
    emailWorker.on("completed", (job) =>
      logger.debug({ jobId: job?.id }, "email worker completed")
    );
