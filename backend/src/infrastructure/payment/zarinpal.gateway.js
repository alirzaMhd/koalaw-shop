...
class ZarinpalGateway {
    ...
    constructor(config) {
        ...
        logger.info({ merchantId: merchantId.slice(0, 8) + "***", sandbox }, "Zarinpal gateway initialized");
    }
    // ==================== Payment Methods ====================
    async createPayment(request) {
        ...
        if (request.amount < minAmount) {
            throw new AppError(`مبلغ باید بیشتر از ${minAmount.toLocaleString()} ${request.currency === "IRT" ? "تومان" : "ریال"} باشد.`, 400, "AMOUNT_TOO_LOW");
        }
        try {
            const payload = {
    ...
    async inquireTransaction(request) {
        ...
        try {
            ...
            const response = await this.client.post("/inquiry.json", payload);
            const data = response.data?.data;
            const errors = response.data?.errors;
            if (errors || !data) {
                const code = errors?.code || -1;
    ...
    async getUnverifiedTransactions() {
        try {
            ...
            const response = await this.client.post("/unVerified.json", payload);
            const data = response.data?.data;
            const errors = response.data?.errors;
            if (errors || !data) {
                const code = errors?.code || -1;
    ...
    async reverseTransaction(request) {
        ...
        try {
            ...
            const response = await this.client.post("/reverse.json", payload);
            const data = response.data?.data;
            const errors = response.data?.errors;
            if (errors || !data || data.code !== 100) {
                const code = errors?.code || data?.code || -1;
    ...
    async createRefund(request) {
        ...
        try {
            ...
            const response = await this.advancedClient.post("/api/v1/refunds", payload);
    ...
    async getRefund(refundId) {
        ...
        try {
            const response = await this.advancedClient.get(`/api/v1/refunds/${refundId}`);
            return response.data;
        }
        catch (error) {
    ...
    async listTransactions(request) {
        ...
        try {
            ...
            const response = await this.advancedClient.get("/api/v1/transactions", { params });
            return response.data;
        }
        catch (error) {
    ...
    async calculateFee(request) {
        try {
            ...
            const response = await this.client.post("/feeCalculation.json", payload);
            const data = response.data?.data;
            const errors = response.data?.errors;
            if (errors || !data || data.code !== 100) {
                const code = errors?.code || data?.code || -1;
    ...
    async createPaymentIntent(args) {
        ...
        const currency = args.currency === "IRT" ? "IRT" : "IRR";
...
//# sourceMappingURL=zarinpal.gateway.js.map