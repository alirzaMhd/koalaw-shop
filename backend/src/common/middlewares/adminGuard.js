...
export const adminGuard = (req, _res, next) => {
    try {
        ...
        if (!isAdmin && !isStaff) {
            ...
            throw AppError.forbidden("دسترسی محدود. این بخش فقط برای مدیران سیستم در دسترس است.");
        }
 ...
 */
export const strictAdminGuard = (req, _res, next) => {
    try {
        if (!req.user) {
            throw AppError.unauthorized("لطفاً ابتدا وارد شوید.");
        }
        ...
        if (normalizedRole !== "ADMIN") {
            ...
            throw AppError.forbidden("دسترسی محدود. این عملیات فقط برای ادمین‌های اصلی مجاز است.");
        }
        logger.info({
            userId: req.user.id || req.user.sub,
            role: normalizedRole,
    ...
    return (req, _res, next) => {
        try {
            ...
            if (!normalizedRoles.includes(normalizedUserRole)) {
                ...
                throw AppError.forbidden(`دسترسی محدود. نقش‌های مجاز: ${allowedRoles.join(", ")}`);
            }
            logger.debug({
                userId: req.user.id || req.user.sub,
                role: normalizedUserRole,
 ...
 */
export const isStrictAdmin = (user) => {
    if (!user || !user.role)
        return false;
    const role = user.role.trim().toUpperCase();
    return role === "ADMIN";
