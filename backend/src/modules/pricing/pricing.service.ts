...
export interface QuoteResult {
  ...
  appliedCoupon?: {
    ...
    ok: boolean;
    reason?: string;
    effect?: { discount: number; shippingDiscount: number };
    id?: string;
  };
}
...
function sampleCoupon(code: string): Coupon | null {
  const upper = normalizeCouponCode(code);
  const now = new Date();
  const base: Partial<Coupon> = {
    minSubtotal: 0,
    isActive: true,
    createdAt: now,
    updatedAt: now,
...
function pickCurrency(lines: QuoteLine[], fallback?: string): string {
  const fromLine = lines.find((l) => l.currencyCode)?.currencyCode;
  return fromLine || fallback || DEFAULT_CURRENCY;
}

async function loadCouponByCode(code: string): Promise<Coupon | null> {
  const upper = normalizeCouponCode(code);
...
async function countCouponRedemptions(couponId: string, userId?: string | null) {
...
function computeFromLines(
  rawLines: QuoteLine[],
  opts: QuoteOptions,
  couponEffect?: { discount: number; shippingDiscount: number }
): QuoteResult {
...
class PricingService {
   ...
   */
  async quoteCart(cartId: string, opts: QuoteOptions = {}): Promise<QuoteResult> {
   ...
   */
  async quoteLines(lines: QuoteLine[], opts: QuoteOptions = {}): Promise<QuoteResult> {
    let appliedCoupon:
      | { code: string; ok: boolean; reason?: string; effect?: { discount: number; shippingDiscount: number }; id?: string }
      | undefined;

    let effect: { discount: number; shippingDiscount: number } | undefined;
