...
function parseOverrides() {
    ...
    try {
        ...
        return arr
            .map((o) => {
            const ro = { rate: Number(o.rate) };
            if (o.country)
                ro.country = String(o.country).toUpperCase();
            if (o.province)
                ro.province = String(o.province).toLowerCase();
            if (o.city)
                ro.city = String(o.city).toLowerCase();
            if (o.postalPrefix)
                ro.postalPrefix = String(o.postalPrefix);
            return ro;
        })
...
function resolveRate(ctx) {
    ...
    let match = candidates.find((o) => (!!o.province ? o.province === province : true) &&
        (!!o.city ? o.city === city : true) &&
        (!!o.postalPrefix ? postal.startsWith(o.postalPrefix) : true)) ||
        // province + postalPrefix
        candidates.find((o) => (!!o.province ? o.province === province : true) &&
            !o.city &&
            (!!o.postalPrefix ? postal.startsWith(o.postalPrefix) : true)) ||
        // province + city
...
function lineAmount(l) {
    if (typeof l.lineTotal === "number")
        return roundMoney(l.lineTotal);
    ...
    computeForLines(input, ctx) {
        ...
        for (const l of input.lines || []) {
            ...
            if (include) {
                ...
                const lt = { base: amount, ratePct, tax };
                if (typeof l.id === "string")
                    lt.id = l.id;
                lineTaxes.push(lt);
            }
