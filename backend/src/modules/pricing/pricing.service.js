...
// Config from env
const FREE_SHIP_THRESHOLD = Number(
  env.PRICING_FREE_SHIP_THRESHOLD ?? 1_000_000
);
...
const ENABLE_SAMPLE_COUPONS =
  String(env.PRICING_ENABLE_SAMPLE_COUPONS ?? "true") === "true";
...
function computeFromLines(rawLines, opts, couponEffect) {
  const lines = rawLines.map((l) => ({
    ...
    lineTotal: Math.max(
      0,
      Math.floor(l.unitPrice || 0) * Math.max(1, Math.floor(l.quantity || 1))
    ),
  }));
  ...
  const discount = Math.min(
    subtotal,
    Math.max(0, Math.floor(couponEffect?.discount || 0))
  );
  ...
  const shippingDiscount = Math.min(
    shippingBase,
    Math.max(0, Math.floor(couponEffect?.shippingDiscount || 0))
  );
  const method = normalizeMethod(opts.shippingMethod);
  const shipping =
    Math.max(0, shippingBase - shippingDiscount) +
    (method === "express" ? EXPRESS_SURCHARGE : 0);
    ...
    if (opts.couponCode) {
      ...
      if (coupon && isCouponActive(coupon)) {
        ...
        const subtotal = lines.reduce(
          (acc, l) =>
            acc +
            Math.max(0, Math.floor(l.unitPrice || 0)) *
              Math.max(1, Math.floor(l.quantity || 1)),
          0
        );
        ...
        const prelimShippingBase =
          subtotal >= FREE_SHIP_THRESHOLD ? 0 : BASE_SHIPPING;
