...
async function searchMagazinePostsWithElasticsearch(opts: {
  ...
  logger.info(
    { total, page, totalPages, took: (res as any).took },
    "Magazine search completed (Elasticsearch)"
  );
...
async function searchMagazinePostsWithPrisma(opts: {
  ...
  const items = posts.map((post: { id: any; slug: any; title: any; excerpt: any; content: any; category: any; authorId: any; author: { name: any; }; tags: any[]; heroImageUrl: any; readTimeMinutes: any; isPublished: any; publishedAt: any; createdAt: any; updatedAt: any; }) => ({
    id: post.id,
    slug: post.slug,
    title: post.title,
    excerpt: post.excerpt,
