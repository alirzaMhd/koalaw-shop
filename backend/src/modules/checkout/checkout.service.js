...
class CheckoutService {
  ...
  async createOrderFromCart(args) {
    ...
    const status = paymentMethod === "cod" ? "processing" : "awaiting_payment";
    ...
    if (paymentMethod === "gateway") {
      ...
      if (provider === "stripe") {
        if (!stripeGateway) {
          logger.error(
            "Stripe gateway not configured. Provide src/infrastructure/payment/stripe.gateway.ts"
          );
          throw new AppError(
            "پرداخت درگاه موقتاً در دسترس نیست.",
            503,
            "PAYMENT_UNAVAILABLE"
          );
        }
        ...
        if (!paypalGateway) {
          logger.error(
            "PayPal gateway not configured. Provide src/infrastructure/payment/paypal.gateway.ts"
          );
          throw new AppError(
            "پرداخت درگاه موقتاً در دسترس نیست.",
            503,
        ...
        const pp = await paypalGateway.createOrder({
          ...
          currency,
          returnUrl:
            args.returnUrl || `${env.APP_URL || ""}/payments/paypal/return`,
          cancelUrl:
            args.cancelUrl || `${env.APP_URL || ""}/payments/paypal/cancel`,
