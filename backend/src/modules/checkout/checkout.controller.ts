...
const isUUID = (v: any) => UUID_RE.test(String(v || ""));
function normalizeClientLines(arr: any[] = []) {
  return arr
    .map((l) => {
      const title = String(l?.title || "").trim();
      const unitPrice = Number(l?.unitPrice ?? l?.price ?? 0);
      const quantity = Math.max(1, Math.floor(Number(l?.quantity ?? l?.qty ?? 1)));
      if (!title || !Number.isFinite(unitPrice) || unitPrice < 0 || quantity <= 0) return null;
      return {
        title,
        unitPrice,
        quantity,
...
const createOrderSchema = baseCreateOrderSchema.transform((v) => {
  const lines =
    (Array.isArray(v.lines) && v.lines.length && v.lines) ||
