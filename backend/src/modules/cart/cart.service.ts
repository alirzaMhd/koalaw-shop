...
function lineTotal(unitPrice: number, qty: number) {
...
class CartService {
  ...
  async getById(cartId: string): Promise<CartWithItems> {
  ...
  async getOrCreateForAnonymous(anonymousId: string): Promise<CartWithItems> {
  ...
  async addItem(cartId: string, input: { productId: string; variantId?: string | null; quantity?: number }) {
  ...
  async updateItem(cartId: string, itemId: string, input: { quantity?: number }) {
    ...
    if (!item || item.cartId !== cartId) throw new AppError("آیتم سبد یافت نشد.", 404, "ITEM_NOT_FOUND");
  ...
  async quote(cartId: string, opts: QuoteOptions = {}): Promise<QuoteResult> {
  ...
  async setStatus(cartId: string, status: CartStatus) {
