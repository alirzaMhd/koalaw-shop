...
async function tryLoadStripe() {
  try {
    ...
    logger.warn(
      "Stripe SDK not installed or STRIPE_SECRET_KEY missing; skipping strict webhook verification."
    );
    return null;
  }
}
async function findPaymentByAuthorityOrOrder(opts) {
  if (opts.authority) {
...
class PaymentService {
  ...
  async markPaid(args) {
    ...
    return orderService.markPaymentSucceeded({
      orderId,
      paymentId,
  ...
  async markFailed(args) {
    ...
    return orderService.markPaymentFailed({
      orderId,
      paymentId,
      reason: reason ?? null,
      transactionRef: transactionRef ?? null,
  ...
  async refund(args) {
    ...
    if (p.status !== "paid")
      throw new AppError(
        "بازپرداخت تنها برای پرداخت‌های موفق مجاز است.",
        409,
        "BAD_STATE"
      );
  ...
  async handleStripeWebhook(opts) {
    ...
    if (!event) {
      ...
      try {
        event =
          typeof opts.rawBody === "string"
            ? JSON.parse(opts.rawBody)
            : JSON.parse(opts.rawBody.toString("utf8"));
        logger.warn(
          "Stripe webhook handled without signature verification (dev mode)."
        );
      } catch {
    ...
    const intent =
      obj.object === "payment_intent" ? obj : obj.payment_intent || obj;
    ...
    const amountReceived =
      intent?.amount_received ?? obj?.amount ?? obj?.amount_total;
    const currencyCode = (intent?.currency || obj?.currency || "")
      .toString()
      .toUpperCase();
    if (!orderId && !authority) {
      logger.warn(
        { type },
        "Stripe event missing orderId/authority; ignoring."
      );
      return { ok: true };
    }
    const payment = await findPaymentByAuthorityOrOrder({
      authority,
    ...
    if (!payment) {
      logger.warn(
        { orderId, authority },
        "Stripe webhook: matching payment not found"
      );
      return { ok: true };
    }
    if (
    ...
    if (
      ...
      await orderService.markPaymentFailed({
        ...
        paymentId: payment.id,
        reason:
          event?.data?.object?.last_payment_error?.message || "payment_failed",
        transactionRef: latestCharge ?? null,
        authority: authority ?? null,
      });
      return { ok: true };
  ...
  async handlePaypalWebhook(opts) {
    ...
    if (!payment) {
      logger.warn(
        { orderId, authority },
        "PayPal webhook: matching payment not found"
      );
      return { ok: true };
    }
    if (
  ...
  async handleGenericGatewayReturn(args) {
    const payment = await findPaymentByAuthorityOrOrder({
      authority: args.authority || undefined,
      orderId: args.orderId || null,
    });
    if (!payment)
      throw new AppError("پرداخت مرتبط یافت نشد.", 404, "PAYMENT_NOT_FOUND");
    if (args.success) {
      await orderService.markPaymentSucceeded({
        orderId: payment.orderId,
        paymentId: payment.id,
