...
export const authService = {
  ...
  async login(args) {
    ...
    const { token: accessToken, expiresAt: accessTokenExpiresAt } =
      signAccessToken(String(user.id), user.role);
    const {
      token: refreshToken,
  ...
  async verifyEmail(args) {
    ...
    if (!storedCode) {
      throw new AppError(
        "کد تایید منقضی شده یا یافت نشد.",
        400,
        "CODE_EXPIRED"
      );
    }
  ...
  async resetPassword(args) {
    ...
    if (!storedCode) {
      throw new AppError(
        "کد بازیابی منقضی شده یا یافت نشد.",
        400,
        "CODE_EXPIRED"
      );
    }
  ...
  async refresh(args) {
    ...
    const { token: accessToken, expiresAt: accessTokenExpiresAt } =
      signAccessToken(userId, user.role);
    const {
      token: refreshToken,
      expiresAt: refreshTokenExpiresAt,
      jti: newJti,
  ...
  async logout(args) {
    ...
    if (args.all) {
      try {
        ...
        logger.info(
          { userId: args.userId, count },
          "Revoked all sessions for user"
        );
        revoked = count > 0;
      } catch (err) {
        logger.error(
          { err, userId: args.userId },
