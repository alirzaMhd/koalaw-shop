...
async function storeRefreshSession(jti, payload) {
...
async function revokeRefreshSession(jti) {
    const key = `session:rt:${jti}`;
...
async function deletePasswordResetCode(email) {
    const key = `password:reset:${email}`;
    await redis.del(key);
}
export const authService = {
    ...
    async login(args) {
        ...
        // Generate tokens
        const { token: accessToken, expiresAt: accessTokenExpiresAt } = signAccessToken(String(user.id), user.role);
    ...
    async resendVerificationCode(args) {
        const { email } = args;
        // Check if user exists
        const user = await prisma.user.findUnique({ where: { email } });
        if (!user) {
    ...
    async refresh(args) {
        ...
        const { token: accessToken, expiresAt: accessTokenExpiresAt } = signAccessToken(userId, user.role);
