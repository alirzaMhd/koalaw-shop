...
class UserService {
  ...
  async getNotificationPrefs(userId) {
    let row = await prisma.userNotificationPrefs.findUnique({
      where: { userId },
    });
    if (!row) {
  ...
  async createAddress(userId, input) {
    ...
    eventBus.emit("user.address.created", {
      userId,
      addressId: entity.id,
    ...
    const row = await prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.userAddress.update({
        ...
        data: {
          ...
          postalCode:
            typeof input.postalCode !== "undefined"
              ? input.postalCode
              : undefined,
          ...
          addressLine2:
            typeof input.addressLine2 !== "undefined"
              ? input.addressLine2
              : undefined,
          country: input.country ?? undefined,
          isDefault:
            typeof input.isDefault !== "undefined" ? willBeDefault : undefined,
        },
      });
      return updated;
    });
    const entity = mapDbAddressToEntity(row);
    eventBus.emit("user.address.updated", {
      userId,
      addressId,
      isDefault: entity.isDefault,
    });
  ...
  async deleteAddress(userId, addressId) {
    ...
    eventBus.emit("user.address.deleted", {
      userId,
      addressId,
      reassignedDefault: reassigned,
    });
