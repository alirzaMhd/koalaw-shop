...
export function mapDbProductToEntity(row) {
  ...
  const dbCategory = row.dbCategory
    ...
    : row.categoryId || row.category_id
      ? mapDbCategoryToRef({
          ...
          heroImageUrl:
            row.categoryHeroImageUrl ?? row.category_hero_image_url ?? null,
        })
      : null;
  ...
  return {
    ...
    isSpecialProduct: Boolean(
      row.isSpecialProduct ?? row.is_special_product ?? false
    ),
    isActive: Boolean(row.isActive ?? row.is_active ?? true),
...
export function toProductCardDto(p) {
  ...
  const categoryLabel =
    p.dbCategory?.label ?? fallbackCategoryLabel(p.category);
  const categoryHeroImageUrl = p.dbCategory?.heroImageUrl ?? null;
  const categoryIcon =
    p.dbCategory?.icon ?? fallbackCategoryIcon(p.category) ?? null;
  return {
    id: p.id,
    slug: p.slug,
    title: p.title,
