  ...
  async create(input) {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const relatedIds = Array.from(
        new Set(relatedIdsRaw.filter((rid) => rid && rid !== product.id))
      );
  ...
  async update(id, input) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (Array.isArray(input.relatedProductIds)) {
        const relatedIds = Array.from(
          new Set(input.relatedProductIds.filter((rid) => rid && rid !== id))
        );
  ...
  async updateVariant(productId, variantId, input /* UpdateVariantInput */) {
    ...
    const row = await prisma.productVariant.update({
      ...
      data: {
        ...
        ...(input.colorHexCode !== undefined && {
          colorHexCode: input.colorHexCode,
        }),
