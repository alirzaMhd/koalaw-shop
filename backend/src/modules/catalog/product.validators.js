...
const variantInputSchema = z.object({
  variantName: z
    .string()
    .min(1, "نام واریانت الزامی است")
    .max(100, "نام واریانت حداکثر ۱۰۰ کاراکتر"),
  ...
  colorHexCode: z
    .string()
    .regex(/^#[0-9A-Fa-f]{6}$/, "کد رنگ باید به فرمت hex باشد")
    .optional(),
...
export const createProductInputSchema = z
  .object({
    ...
    images: z
      ...
      .max(20, "حداکثر ۲۰ تصویر مجاز است")
      .optional(),
    variants: z
      .array(variantInputSchema)
      .max(50, "حداکثر ۵۰ واریانت مجاز است")
      .optional(),
    badgeIds: z.array(z.string().uuid()).optional(),
    relatedProductIds: z
      .array(z.string().uuid("شناسه محصول مرتبط نامعتبر است"))
      .optional(),
  })
  .refine((data) => data.brandId || data.brandSlug, {
    message: "شناسه یا اسلاگ برند الزامی است",
    path: ["brandId"],
  });
...
export const updateImageInputSchema = z.object({
  ...
  alt: z
    .string()
    .max(255, "متن جایگزین تصویر حداکثر ۲۵۵ کاراکتر")
    .nullable()
    .optional(),
  position: positionSchema,
});
...
export const validateProductInput = {
  list: (data) => listProductsQuerySchema.parse(data),
  create: (data) => createProductInputSchema.parse(data),
  update: (data) => updateProductInputSchema.parse(data),
  addImage: (data) => addImageInputSchema.parse(data),
  updateImage: (data) => updateImageInputSchema.parse(data),
  addVariant: (data) => addVariantInputSchema.parse(data),
  updateVariant: (data) => updateVariantInputSchema.parse(data),
  addReview: (data) => addReviewInputSchema.parse(data),
  listReviews: (data) => listReviewsQuerySchema.parse(data),
};
