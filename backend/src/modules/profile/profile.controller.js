...
class ProfileController {
  ...
  updateNotificationPrefs = async (req, res, next) => {
    try {
      ...
      const updated = await profileService.updateNotificationPrefs(
        String(userId),
        prefs
      );
    ...
    try {
      ...
      if (!file) {
        return ok(
          ...
          {
            message:
              "در حال حاضر تصویر پیش‌فرض کوالای کوچولو برای شما تنظیم شده است! 🐨💕",
  ...
  setDefaultAddress = async (req, res, next) => {
    try {
      ...
      const result = await profileService.setDefaultAddress(
        String(userId),
        addressId
      );
      return ok(res, result, 200);
