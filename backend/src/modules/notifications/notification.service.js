...
try {
  ...
  const m = require("../../infrastructure/mail/mailer.js");
...
const MAIL_FROM =
  env.MAIL_FROM ||
  `no-reply@${(env.APP_URL || "example.com").replace(/^https?:\/\//, "")}`;
// Paths for templates
...
async function renderTemplate(name, data) {
  try {
    ...
    logger.warn(
      { err: e, name },
      "Template rendering failed; falling back to inline template."
    );
  }
...
async function sendViaMailer(payload) {
  if (!mailer) {
    logger.info(
      ...
      "Mailer missing. Logging email payload:"
    );
...
async function enqueueEmailJob(payload) {
  try {
    ...
    await q.add("email.send", payload, {
...
class NotificationService {
  ...
  async sendEmail(payload, opts) {
    ...
    if (payload.templateName) {
      rendered = await renderTemplate(payload.templateName, {
  ...
  async sendSms(payload) {
    if (!smsProvider || typeof smsProvider.send !== "function") {
      logger.info(
        { to: payload.to, text: payload.text },
        "SMS provider missing. Logging SMS payload."
      );
      return;
    }
  ...
  async sendOrderConfirmation(orderId) {
    ...
    if (!toEmail) {
      ...
      logger.info(
        { orderId },
        "Order confirmation email skipped (no user email)."
      );
      return;
    }
    const name =
  ...
  async sendPaymentReceipt(orderId, paymentId) {
    ...
    if (!toEmail) {
      logger.info(
        { orderId },
        "Payment receipt email skipped (no user email)."
      );
      return;
    }
    const name =
    ...
    await this.sendEmail(
      {
        ...
        templateData: {
          ...
          title: "رسید پرداخت",
          greeting: name
            ? `${name} عزیز، پرداخت شما با موفقیت انجام شد.`
  ...
  async sendShippingUpdate(orderId, args) {
    ...
    if (!toEmail) {
      logger.info(
        { orderId },
        "Shipping update email skipped (no user email)."
      );
      return;
    }
    const name =
    ...
    await this.sendEmail(
      {
        ...
        templateData: {
          ...
          greeting: name
            ? `${name} عزیز، سفارش شما در حال ارسال است.`
  ...
  bindDefaultHandlers() {
    ...
    onPaymentSucceeded((evt) => {
      this.sendPaymentReceipt(evt.orderId, evt.paymentId).catch((e) =>
        logger.warn(
          { err: e, orderId: evt.orderId, paymentId: evt.paymentId },
