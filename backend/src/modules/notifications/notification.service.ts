...
export interface OrderEmailContext {
  ...
  items: Array<{ title: string; variantName?: string | null; qty: number; unitPrice: number; lineTotal: number; imageUrl?: string | null }>;
  shipping: { method: string; province?: string | null; city?: string | null; addressLine1?: string | null; postalCode?: string | null };
...
async function renderTemplate(name: string, data: Record<string, any>): Promise<{ html: string; text?: string }> {
  try {
...
async function sendViaMailer(payload: EmailPayload): Promise<void> {
  if (!mailer) {
...
async function enqueueEmailJob(payload: EmailPayload): Promise<boolean> {
  try {
    const q = queues?.email || queues?.mailer || queues?.default;
    if (!q || typeof q.add !== "function") return false;
...
class NotificationService {
  ...
  async sendOrderConfirmation(orderId: string): Promise<void> {
    const order = await prisma.order.findUnique({
      ...
      include: {
        ...
        user: { select: { email: true, firstName: true, lastName: true, phone: true } } as any,
      },
    });
    ...
    const ctx: OrderEmailContext = {
      ...
      shipping: {
        ...
        province: (order as any).shippingProvince ?? null,
        city: (order as any).shippingCity ?? null,
        addressLine1: (order as any).shippingAddressLine1 ?? null,
        postalCode: (order as any).shippingPostalCode ?? null,
      },
   ...
   */
  async sendPaymentReceipt(orderId: string, paymentId: string): Promise<void> {
    const [order, payment] = await Promise.all([
      prisma.order.findUnique({
        where: { id: orderId },
        include: { items: true, user: { select: { email: true, firstName: true, lastName: true } } as any },
      }),
      prisma.payment.findUnique({ where: { id: paymentId } }),
    ]);
    if (!order || !payment) throw new AppError("سفارش/پرداخت یافت نشد.", 404, "NOT_FOUND");
    const toEmail = order.user?.email || null;
    if (!toEmail) {
  ...
  bindDefaultHandlers() {
    onOrderCreated((evt) => {
      this.sendOrderConfirmation(evt.orderId).catch((e) =>
        logger.warn({ err: e, orderId: evt.orderId }, "Failed to send order confirmation")
      );
    });
    onPaymentSucceeded((evt) => {
      this.sendPaymentReceipt(evt.orderId, evt.paymentId).catch((e) =>
        logger.warn({ err: e, orderId: evt.orderId, paymentId: evt.paymentId }, "Failed to send payment receipt")
      );
    });
