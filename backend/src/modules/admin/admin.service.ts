...
export const adminService = {
  ...
  async getDashboardStats() {
    ...
    return {
      ...
      recentOrders: recentOrders.map((order: { id: any; orderNumber: any; status: any; total: any; createdAt: any; user: any; }) => ({
        id: order.id,
  ...
  async updateUserTier(userId: string, tier: string) {
    ...
    const user = await prisma.user.update({
      where: { id: userId },
      data: { customerTier: tier.toUpperCase() as any },
    });
  ...
  // ========== REVIEW MODERATION ==========
  async listPendingReviews(query: { page?: number; perPage?: number }) {
    const page = Math.max(1, query.page || 1);
    const perPage = Math.min(100, Math.max(1, query.perPage || 20));
    const skip = (page - 1) * perPage;
  ...
  async createBrand(data: { name: string; slug?: string }) {
  ...
  async updateBrand(id: string, data: { name?: string; slug?: string }) {
  ...
  async updateCollection(id: string, data: { name: string }) {
  ...
  // ========== COUPON MANAGEMENT ==========
  async listCoupons(query: { page?: number; perPage?: number }) {
    const page = Math.max(1, query.page || 1);
    const perPage = Math.min(100, Math.max(1, query.perPage || 20));
    const skip = (page - 1) * perPage;
