...
export interface RateQuote {
  ...
  amount: number;
  currencyCode: string;
  eta: string; // e.g., "2-4 days"
  free: boolean;
  base: number; // base part before surcharge
  surcharge: number; // express surcharge if any
...
type RegionOverride = {
  province?: string; // normalized lower-case
  multiplier?: number; // multiply base
  extra?: number; // add after multiplier (flat)
};
...
function applyRegion(base: number, province?: string): number {
...
function etaFor(method: ShippingMethod): string {
  return method === "express" ? ETA_EXP : ETA_STD;
}

// ---------- Core compute ----------

function computeBaseShipping(subtotal: number, province?: string, couponFreeShip?: boolean): number {
 ...
 */
function quoteForSubtotal(subtotal: number, addr?: Address, opts?: QuoteOptions): ShippingQuotes {
...
class ShippingService {
   ...
   */
  async quoteCart(cartId: string, addr?: Address, opts?: QuoteOptions): Promise<ShippingQuotes & { subtotal: number }> {
   ...
   */
  async createShipmentLabel(orderId: string, carrier: string = "local-post"): Promise<ShipmentLabel> {
   ...
   */
  async track(trackingNumber: string): Promise<{ trackingNumber: string; status: string; lastUpdate: string; history: Array<{ at: string; status: string; location?: string }> }> {
  ...
  private generateTrackingNumber(carrier: string) {
