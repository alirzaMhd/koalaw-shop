...
function parseOverrides() {
  try {
    ...
    return arr
      .map((o) => ({
        province: o.province
          ? String(o.province).toLowerCase().trim()
          : undefined,
        multiplier: Number.isFinite(Number(o.multiplier))
          ? Number(o.multiplier)
          : undefined,
    ...
    logger.warn(
      { err: e },
      "Failed to parse SHIPPING_REGION_OVERRIDES; ignoring."
    );
    return [];
  }
}
...
function buildRateQuote(args) {
  const base = computeBaseShipping(
    args.subtotal,
    args.province,
    !!args.couponFreeShip
  );
...
class ShippingService {
  ...
  fromPricingQuote(quote, method, addr) {
    const base = computeBaseShipping(
      quote.postSubtotal,
      addr?.province,
      quote.appliedCoupon?.ok && quote.appliedCoupon.effect?.shippingDiscount
        ? true
        : false
    );
  ...
  async createShipmentLabel(orderId, carrier = "local-post") {
    ...
    logger.info(
      { orderId, orderNumber: order.orderNumber, trackingNumber, carrier },
      "Shipment label created (stub)"
    );
  ...
  async track(trackingNumber) {
    ...
    return {
      ...
      history: [
        {
          at: new Date(now.getTime() - 3 * 24 * 60 * 60 * 1000).toISOString(),
