...
const createLabelSchema = z
  .object({
    orderId: uuid,
    carrier: z.string().trim().min(2).max(40).optional().default("local-post"),
  })
  .strict();
const trackingParamSchema = z.object({
  trackingNumber: z.string().trim().min(4, "کد رهگیری نامعتبر است."),
});
// ------------ Controller ------------
class ShippingController {
  ...
  quote = async (req, res, next) => {
    try {
      ...
      if (body.cartId) {
        const result = await shippingService.quoteCart(
          body.cartId,
          body.address,
          {
  ...
  createLabel = async (req, res, next) => {
    try {
      const { orderId, carrier } = await createLabelSchema.parseAsync(
        req.body ?? {}
      );
  ...
  track = async (req, res, next) => {
    try {
      const { trackingNumber } = await trackingParamSchema.parseAsync(
        req.params
      );
