...
export const newsletterController = {
    ...
    subscribe: (async (req, res, next) => {
        try {
            ...
            const result = await newsletterService.subscribe(parsed.email, parsed.source || "website", ipAddress);
            res.json({
                success: true,
    ...
    unsubscribe: (async (req, res, next) => {
        try {
            ...
            const result = await newsletterService.unsubscribe(parsed.email, parsed.token);
            // If GET request (one-click), redirect to confirmation page
            if (req.method === "GET") {
                return res.redirect(`${process.env.FRONTEND_URL || ""}/newsletter/unsubscribed?email=${encodeURIComponent(parsed.email)}`);
            }
            res.json({
                success: true,
    ...
    getSubscriptions: (async (req, res, next) => {
        try {
            ...
            const result = await newsletterService.getActiveSubscriptions(page, perPage);
            res.json({
                success: true,
                data: result,
            });
