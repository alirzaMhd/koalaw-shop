...
export const newsletterService = {
   ...
   */
  async subscribe(email: string, source?: string, ipAddress?: string) {
    ...
    if (existing) {
      ...
      if (existing.unsubscribedAt) {
        ...
        logger.info(
          { email: normalizedEmail, source, ip: ipAddress },
   ...
   * Queue welcome email
   */
  async queueWelcomeEmail(email: string) {
    try {
   ...
   * Queue unsubscribe confirmation email
   */
  async queueUnsubscribeConfirmationEmail(email: string) {
    try {
      await emailQueue.add(
...
`;
   ...
   */
  async sendUnsubscribeConfirmationEmail(email: string) {
...
`;
  ...
  async sendNewsletter(opts: {
    ...
    for (let i = 0; i < subscriptions.length; i += batchSize) {
      ...
      logger.info(
        { batch: Math.floor(i / batchSize) + 1, emails: batch.length },
        "Newsletter batch queued"
      );
    }

    return {
      sent: subscriptions.length,
      recipients: subscriptions.map((s: { email: any; }) => s.email),
      test: false,
      queued: true,
    };
  },
  ...
  async exportSubscribers() {
    ...
    const csv = [
      ...
      ...subscriptions.map(
        (s: { email: any; source: any; createdAt: { toISOString: () => any; }; }) =>
