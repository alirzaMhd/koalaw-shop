...
const cartParam = z.object({
  cartId: z.string().uuid({ message: "شناسه سبد خرید نامعتبر است." }),
});
...
class InventoryController {
  ...
  reserveCart = async (req, res, next) => {
    try {
      const { cartId } = await cartParam.parseAsync(req.params);
  ...
  reserveOrder = async (req, res, next) => {
    try {
  ...
  releaseOrder = async (req, res, next) => {
    try {
      const { orderId } = await orderParam.parseAsync(req.params);
