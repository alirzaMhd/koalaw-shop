...
export class InventoryService {
    ...
    async setStock(variantId, stock) {
        if (!Number.isInteger(stock) || stock < 0)
            throw new AppError("مقدار موجودی نامعتبر است.", 400, "BAD_STOCK");
    ...
    async adjustStock(variantId, delta) {
        if (!Number.isInteger(delta))
            throw new AppError("تغییر موجودی باید عدد صحیح باشد.", 400, "BAD_STOCK");
        ...
        if (!ALLOW_BACKORDER && delta < 0) {
            ...
            if (current + delta < 0)
                throw new AppError("موجودی کافی نیست.", 409, "OUT_OF_STOCK");
        }
        const v = await prisma.productVariant.update({
            where: { id: variantId },
    ...
    async reserveOne(tx, variantId, qty) {
        if (qty <= 0)
            throw new AppError("تعداد نامعتبر است.", 400, "BAD_QTY");
        if (ALLOW_BACKORDER) {
        ...
        if (res.count === 0) {
            ...
            err.meta = { variantId, requested: qty, available };
            throw err;
        }
    }
    async releaseOne(tx, variantId, qty) {
        if (qty <= 0)
            throw new AppError("تعداد نامعتبر است.", 400, "BAD_QTY");
        await tx.productVariant.update({
            where: { id: variantId },
    ...
    async reserveForOrder(orderId) {
        ...
        if (skipped.length) {
            logger.warn({ orderId, skippedCount: skipped.length }, "reserveForOrder: some order_items have no variantId; skipped inventory reservation");
        }
        return result;
    }
    /**
     ...
     */
    async verifyCartAvailability(cartId) {
        const items = await prisma.cartItem.findMany({
            where: { cartId },
            select: { id: true, variantId: true, quantity: true },
        });
        if (!items.length)
            return { ok: true, lines: [], unavailableCount: 0, missingVariantCount: 0 };
        ...
        const lines = items.map((it) => {
            ...
            const available = stocks.has(it.variantId) ? stocks.get(it.variantId) : 0;
