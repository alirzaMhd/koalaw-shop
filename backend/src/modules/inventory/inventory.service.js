...
const ALLOW_BACKORDER =
  String(env.INVENTORY_ALLOW_BACKORDER || "false") === "true";
export class InventoryService {
  ...
  async adjustStock(variantId, delta) {
    ...
    eventBus.emit("inventory.stock.adjusted", {
      variantId,
      delta,
      stock: v.stock,
    });
  ...
  async reserve(lines) {
    ...
    for (const l of lines) {
      merged.set(
        l.variantId,
        (merged.get(l.variantId) || 0) + (l.quantity || 0)
      );
    }
    const compact = Array.from(merged.entries()).map(
    ...
    try {
      ...
      if (e?.code === "P2025") {
        ...
        e?.code === "OUT_OF_STOCK" ||
        e?.message?.includes("موجودی کافی نیست")
      ) {
        const meta = e.meta;
        if (meta?.variantId) {
          failures.push({
      ...
      throw e instanceof AppError
        ? e
        : new AppError("عدم امکان رزرو موجودی.", 409, "RESERVE_FAILED");
    }
  }
  /**
