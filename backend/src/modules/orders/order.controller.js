...
class OrderController {
    ...
    listMine = async (req, res, next) => {
        try {
            ...
            const cleanQ = Object.fromEntries(Object.entries(q).filter(([, v]) => v !== undefined));
    ...
    getMineById = async (req, res, next) => {
        try {
            const userId = req.user?.id || req.user?.sub;
            if (!userId)
                throw new AppError("احراز هویت انجام نشد.", 401, "UNAUTHORIZED");
    ...
    markPaymentSucceeded = async (req, res, next) => {
        try {
    ...
    markPaymentFailed = async (req, res, next) => {
        try {
            const { id, paymentId } = await z
                .object({ id: uuidSchema, paymentId: uuidSchema })
                .parseAsync(req.params);
