...
class ReviewService {
    ...
    async create(args) {
        ...
        if (isUser) {
            ...
            if (exists)
                throw new AppError("شما قبلاً برای این محصول نظر ثبت کرده‌اید.", 409, "REVIEW_EXISTS");
        }
    ...
    async updateContent(args) {
        ...
        if (!canEdit)
            throw new AppError("اجازه ویرایش ندارید.", 403, "FORBIDDEN");
    ...
    async delete(id, args) {
        ...
        if (!canDelete)
            throw new AppError("اجازه حذف ندارید.", 403, "FORBIDDEN");
