...
class ReviewService {
  ...
  async listForProduct(productId, query) {
  ...
  async create(args) {
    ...
    if (!isUser && !guestName) {
      throw new AppError(
        "برای ثبت نظر، ورود یا درج نام مهمان لازم است.",
        400,
        "BAD_REVIEWER"
      );
    }
  ...
  async updateContent(args) {
    ...
    const rating =
      typeof args.rating === "number" ? clampRating(args.rating) : undefined;
  ...
  async delete(id, args) {
    ...
    const isOwner = !!ownerId && ownerId === (args?.requesterUserId ?? null);
    const canDelete =
      args?.isAdmin || (isOwner && review.status !== "approved");
  ...
  async recalcProductRating(productId) {
    ...
    const avg = Math.max(
      0,
      Math.min(5, Math.round((agg._avg.rating || 0) * 100) / 100)
    ); // 2 decimals
